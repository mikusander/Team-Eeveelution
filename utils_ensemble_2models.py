...
MODIFIED: This version is adapted to run using ONLY LightGBM and XGBoost.
All CatBoost references have been removed.
"""
...
SUBMISSION_FILE_LOGREG = SUBMISSION_DIR / 'submission_stacking_logreg_2models.csv'
...
BLENDED_SUBMISSION_CLASS_OUT = SUBMISSION_DIR / 'submission_blended_LGBM_XGB_CLASS.csv'
...
def load_oof_data():
    ...
    try:
        ...
        if not (len(oof_lgbm) == len(y_meta) and 
            ...
            raise ValueError(f"OOF length mismatch! LGBM({len(oof_lgbm)}), XGB({len(oof_xgboost)}), Y({len(y_meta)})")

        # Stack only LGBM and XGBoost
        X_meta = np.column_stack((oof_lgbm, oof_xgboost))
...
def load_test_data():
    ...
    try:
        ...
        if not (len(test_preds_lgbm) == len(test_preds_xgboost)):
        ...
        # Stack only LGBM and XGBoost
        X_test_meta = np.column_stack((test_preds_lgbm, test_preds_xgboost))
        ...
        print("Make sure you have generated test set predictions from ALL 2 base models.")
        return None, None
    except Exception as e:
...
def train_evaluate_logreg(X_meta, y_meta):
    ...
    print("LEARNED WEIGHTS (Coefficients) for [LGBM, XGBoost]:")
...
def train_and_select_best_model(X_meta, y_meta):
    ...
    # --- Model Definitions ---
    # These models will train on the 2-column X_meta
    model_logreg = LogisticRegression(random_state=SEED, C=1.0, solver='liblinear')
...
def run_blending_ensemble():
    """
    [BLENDING - Averaging]
    Runs the simple 2-model probability blending (LGBM + XGB).
    Loads CSVs, merges, calculates weighted average, and saves.
    """
    print("\n" + "="*30)
    print("START PHASE: Simple Blending Ensemble (LGBM+XGB)")
    print("="*30)
    
    # Weights for blending (2 models)
    LGBM_WEIGHT = 0.5
    XGBOOST_WEIGHT = 0.5
    ...
    try:
        ...
        print(f"ERROR: File not found: {e}")
        print("Make sure you have generated BOTH probability .csv files.")
        print(f"(Check: '{LGBM_PROBA_FILE}', '{XGBOOST_PROBA_FILE}')")
        return False
    except Exception as e:
    ...
    try:
        ...
        if len(blended_df) != len(lgbm_df):
            ...
            print("Check that all submission files have the same number of rows.")

    except KeyError as e:
        print(f"ERROR: Missing column: {e}")
        print("Ensure the CSVs contain 'battle_id' and 'player_won_proba'.")
        return False
    except Exception as e:
        print(f"ERROR during merge: {e}")
        return False

    print("Calculating weighted average (LGBM + XGB)...")
