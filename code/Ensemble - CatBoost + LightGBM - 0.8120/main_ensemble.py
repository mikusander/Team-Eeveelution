...
def predict_ensemble(base_path: str):
    ...
    ENCODERS_DIR = os.path.join(base_path, "Encoders - LightGBM")
    ...
    for fold in range(1, 11):
        ...
        encoder_filename = os.path.join(ENCODERS_DIR, f"encoder_fold_{fold}.joblib")
        try:
            model = joblib.load(model_filename)
    ...
    WEIGHT_CATBOOST = 0.60
    WEIGHT_LGBM = 0.40
    
    final_preds = (WEIGHT_CATBOOST * catboost_final_preds) + (WEIGHT_LGBM * lgbm_final_preds)
    print(f"Previsioni combinate con pesi: CatBoost={WEIGHT_CATBOOST*100}%, LightGBM={WEIGHT_LGBM*100}%")
