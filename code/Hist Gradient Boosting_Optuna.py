...
def make_features(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # These engineered features capture offensive/defensive balance, speed advantage, and HP resilience advantage
    # engineered battle advantage features
    out['p1_off_def_ratio'] = (out.get('p1_base_atk_mean',0)+out.get('p1_base_spa_mean',0)) / (out.get('p1_base_def_mean',0)+out.get('p1_base_spd_mean',0)+1e-6)
    out['speed_advantage'] = out.get('p1_base_spe_mean',0) - out.get('p2_lead_base_spe',0)
    out['hp_ratio'] = (out.get('p1_base_hp_mean',0)+1e-6) / (out.get('p2_lead_base_hp',0)+1e-6)
...
def train_and_predict(train_path: str, test_path: str, out_path: str, seed: int = 42):
    ...
    # HistGradientBoosting - Optuna hyperparameter search
    optuna_best_params_path = '/Users/lorenzo/Desktop/Università/Sapienza/Computer Science - Magistrale/Foundations of Data Science/Kaggle/Pokemon/Github/Team-Eeveelution/models/params/best_params_hist_gradient_boosting_optuna.json'
    if os.path.exists(optuna_best_params_path):
        with open(optuna_best_params_path, 'r') as f:
            gb_best = json.load(f)
    else:
        print("Running Optuna hyperparameter search for HistGradientBoosting (maximizing accuracy)...")

        def objective(trial):
            # Use a sample of up to 10,000 rows for tuning
            ...
            for tr_idx, va_idx in skf_opt.split(X_sample_scaled, y_sample):
                X_tr, X_va = X_sample_scaled[tr_idx], X_sample_scaled[va_idx]
        ...
        gb_best = study.best_trial.params
        # Add fixed params
        gb_best['early_stopping'] = True
        gb_best['n_iter_no_change'] = 10
        gb_best['validation_fraction'] = 0.1
        gb_best['random_state'] = seed
        print(f"Best params found by Optuna: {gb_best}")
        with open(optuna_best_params_path, 'w') as f:
            json.dump(gb_best, f, indent=4)
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train, y)):
        ...
        # Fit
        clf_fold = HistGradientBoostingClassifier(**gb_best)
    ...
    clf_final = HistGradientBoostingClassifier(**gb_best)
