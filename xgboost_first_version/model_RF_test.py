...
def run_rf_pipeline(train_path: str, test_path: str, output_csv: str):
    ...
    # Usa tutte le feature (o scegli le top N se vuoi)
    ...
    for train_idx, val_idx in skf.split(X_train_top, y_train_main):
        ...
        model_rf = RandomForestClassifier(n_estimators=500, max_depth=8, random_state=42, n_jobs=-1)
        ...
        holdout_preds_ensemble += model_rf.predict_proba(X_holdout_top)[:, 1]
        test_preds_ensemble += model_rf.predict_proba(X_test_top)[:, 1]
    ...
    model_final = RandomForestClassifier(n_estimators=500, max_depth=8, random_state=42, n_jobs=-1)
