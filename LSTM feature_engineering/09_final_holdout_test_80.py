"""
Addestra il modello LSTM Avanzato sull'80% dei dati (Train+Val)
per un numero fisso di epoche letto dal file JSON e valuta sul 20% Holdout.
Genera metriche e grafici. SENZA MLflow.
...
plt.title("LSTM Confusion Matrix (Train 80%, Test Holdout 20%)")
...
plt.title('LSTM ROC-AUC Curve (Train 80%, Test Holdout 20%)')
plt.legend(loc="lower right"); plt.grid(True)
plt.savefig(ROC_CURVE_FILE)
plt.close()
print(f"ROC Curve salvata in: {ROC_CURVE_FILE}")

print(f"\n--- Script 21 (LSTM Holdout 80%, Senza MLflow) Completato ---")
