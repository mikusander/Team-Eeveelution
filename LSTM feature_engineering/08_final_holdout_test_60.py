...
sul 60% Train per un numero fisso di epoche letto dal file JSON
...
try:
    ...
    if STATIC_FEATURES_EXIST:
        print("Caricamento dati statici per Train e Holdout...")
        X_train_static = np.load(X_TRAIN_STATIC_FILE)
        X_holdout_static = np.load(X_HOLDOUT_STATIC_FILE)
        # Aggiungi ai dizionari di input
        X_train_inputs['input_static'] = X_train_static
...
# La chiamata a fit usa X_train_inputs, che è già pronto
...
# La chiamata a predict usa X_holdout_inputs, che è già pronto
...
plt.title(f"LSTM Confusion Matrix (Train 60%, Test Holdout 20%) - Ibrido: {STATIC_FEATURES_EXIST}")
...
plt.title(f"LSTM ROC-AUC Curve (Train 60%, Test Holdout 20%) - Ibrido: {STATIC_FEATURES_EXIST}")
plt.legend(loc="lower right"); plt.grid(True)
plt.savefig(ROC_CURVE_FILE)
plt.close()
print(f"ROC Curve salvata in: {ROC_CURVE_FILE}")

print(f"\n--- Script 08 (LSTM Holdout 60% v5 Ibrido) Completato ---")
